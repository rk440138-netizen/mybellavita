...
import { Star, ChevronDown, Check, Plus, Minus, X } from 'lucide-react';
// 1. Real Data Import
...
const Bestsellers = () => {
  ...
  // --- USE IMPORTED DATA ---
  const products = bestsellersList;
    ...
    <main className="bg-white min-h-screen">
      ...
      <div className="bg-gray-50 py-10 mb-8 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 text-center">
            <h1 className="text-3xl md:text-4xl font-bold uppercase tracking-wide mb-2">Our Bestsellers</h1>
            <p className="text-gray-500 text-sm">Most loved products by our customers.</p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-2xl">
                 <div>
                    ...
                    <ul className="space-y-2 text-sm text-gray-600">
                        {['Luxury Perfume', 'Gift Set', 'Eau De Parfum', 'Cosmetics'].map((cat) => (
                    ...
                    <div key={product.id} className="group bg-white border border-gray-100 hover:shadow-lg transition-all duration-300 rounded-sm overflow-hidden flex flex-col">
                        ...
                        <div className="relative aspect-4/5 bg-gray-50 overflow-hidden">
                            ...
                            <div className="absolute top-2 left-2 flex flex-col gap-1 z-10">
                                <span className={`text-white text-[10px] font-bold px-2 py-0.5 uppercase tracking-wider rounded-sm shadow-sm bg-[#ff9900]`}>{product.tag}</span>
                            </div>
                            
                            <div className="absolute bottom-2 left-2 z-10">
                                <span className="bg-[#e8f5e9] text-[#1b5e20] text-[10px] font-bold px-2 py-0.5 uppercase tracking-wider rounded-sm border border-[#c8e6c9]">
                                   {product.discount}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="p-4 flex flex-col grow">
                            ...
                            <h3 className="text-sm font-bold text-gray-900 line-clamp-2 mb-2 min-h-10 leading-snug group-hover:text-gray-600 transition-colors">
                                {product.title}
                            </h3>
