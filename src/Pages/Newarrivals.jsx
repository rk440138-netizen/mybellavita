...
import { Star, ChevronDown, Check, Plus, Minus, X, Sparkles } from 'lucide-react';
// 1. Import New Arrivals Data
import { newArrivalsList } from '../Data'; 

const NewArrivals = () => {
  // --- SEO & Scroll To Top ---
  useEffect(() => {
    document.title = "New Arrivals | Latest Perfumes & Care | Bella Vita Organic";
    window.scrollTo(0, 0);
  }, []);
  ...
  // --- USE IMPORTED DATA ---
  const products = newArrivalsList;
    ...
    <main className="bg-white min-h-screen">
      ...
      <div className="bg-gray-50 py-12 mb-8 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 text-center">
            ...
            <h1 className="text-3xl md:text-5xl font-bold uppercase tracking-tight mb-3">New Arrivals</h1>
            <p className="text-gray-600 text-sm md:text-base max-w-2xl mx-auto">
                Explore our latest launches in perfumes, skincare, and cosmetics.
            </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 relative z-30">
            ...
            <p className="text-xs text-gray-500 font-bold tracking-widest hidden md:block">{visibleProducts.length} PRODUCTS FOUND</p>
        </div>
        ...
        <div 
          ...
          ${isFilterOpen ? 'max-h-[300px] opacity-100 py-8 px-6 w-full md:w-3/4' : 'max-h-0 opacity-0 py-0 px-0'}`}
        >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-2xl">
                 <div>
                    <h4 className="font-bold text-xs uppercase mb-4 text-black tracking-widest">Category</h4>
                    <ul className="space-y-2 text-sm text-gray-600">
                        {/* Categories extracted from New Arrivals Data */}
                        {['Eau De Parfum', 'Cosmetics', 'Hair Care', 'Body Care'].map((cat) => (
                ...
                <div>
                    <h4 className="font-bold text-xs uppercase mb-4 text-black tracking-widest">Max Price</h4>
                    <div className="px-2">
                    ...
                    <div className="mt-6">
                        <button onClick={() => { setSelectedCategories([]); setPriceRange(2500); }} className="text-xs font-bold underline text-gray-400 hover:text-black transition-colors">Reset All</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-10 md:gap-x-6 md:gap-y-14">
            {visibleProducts.length > 0 ? (
                visibleProducts.map((product) => (
                    <div key={product.id} className="group bg-white border border-gray-100 hover:shadow-xl transition-all duration-500 rounded-sm overflow-hidden flex flex-col">
                        
                        {/* Image Area */}
                        <div className="relative aspect-4/5 bg-gray-50 overflow-hidden">
                            <img src={product.image} alt={product.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 ease-out"/>
                            
                            <div className="absolute top-2 left-2 flex flex-col gap-1 z-10">
                                <span className={`text-white text-[9px] md:text-[10px] font-bold px-2 py-1 uppercase tracking-wider rounded-sm shadow-sm bg-black`}>NEW</span>
                            </div>
                ...
                <div className="col-span-full text-center py-20 text-gray-500">
                    <p className="text-lg">No new arrivals found matching your filters.</p>
                </div>
            )}
        </div>
